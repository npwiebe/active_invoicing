...
RSpec.describe(ActiveInvoicing) do
  # Reset the singleton instance before and after each test
  around do |example|
    Singleton.__init__(ActiveInvoicing::Configuration)
    example.run
    Singleton.__init__(ActiveInvoicing::Configuration)
  end
  ...
  describe ".configure" do
    it "yields the configuration instance" do
      ...
      expect(config_instance).to(be_a(ActiveInvoicing::Configuration))
    end
  ...
  describe ".configuration" do
    it "returns the configuration instance" do
      expect(described_class.configuration).to(be_a(ActiveInvoicing::Configuration))
    end
...
RSpec.describe(ActiveInvoicing::Configuration) do
  # Reset the singleton instance before and after each test
  around do |example|
  ...
  describe "environment variable integration" do
    ...
    it "loads quickbooks_client_id from environment" do
      ...
      expect(ActiveInvoicing.configuration.quickbooks_client_id).to(eq("env_client_id"))
    end
    ...
    it "loads quickbooks_client_secret from environment" do
      ...
      expect(ActiveInvoicing.configuration.quickbooks_client_secret).to(eq("env_client_secret"))
    end
    ...
    it "loads sandbox_mode from environment" do
      ...
      expect(ActiveInvoicing.configuration.sandbox_mode).to(be(true))
    end
    ...
    it "handles missing environment variables" do
      ...
      expect(ActiveInvoicing.configuration.quickbooks_client_id).to(be_nil)
    end
  end
end
